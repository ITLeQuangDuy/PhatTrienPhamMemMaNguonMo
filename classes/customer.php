	...
	class customer
	{
		...
		public function insert_customer($date)
		{
			...
			if($name == "" || $city == "" || $zipcode == "" || $email == "" || $address == "" || $country == "" || $phone == "" || $password == ""){
				...
				if ($result_check) {
					$alert = "<span class='error'>Email đã tồn tại ! Vui lòng nhập địa chỉ email khác </span>";
					return $alert;
				}else {
					...
					if($result){
						...
						$alert = "<span class='error'>Tạo thành khoản không thành công</span>";
						return $alert;
					}
				}
			}
        ...
        public function login_customer($date)
		{
			...
			if($email == '' || $password == ''){
				$alert = "<span class='error'>Không được để trống email và password !</span>";
				return $alert;
			}else{
